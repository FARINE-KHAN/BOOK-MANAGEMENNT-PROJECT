...
const createReviwe = async function (req, res) {
  try {
    ...
    if (!findBook)
      return res
        .status(404)
        .send({ status: false, message: "Book not Available" });
    ...
    res
      .status(201)
      .send({ status: true, message: "success", data: createdReview });
  } catch (error) {
  ...
  try {
    ...
    if (book) {
      if (book["isDeleted"] == true)
        return res
          .status(400)
          .send({ status: false, message: "The book has been deleted" });
    } else
    ...
    if (review.bookId !== bookId) {
      return res
        ...
        .send({
          status: false,
          msg: "review id not match for this book please provide valid review id",
        });
    }
